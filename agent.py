...
ПОРЯДОК РАБОТЫ. Два принципа держи ОДНОВРЕМЕННО, не в ущерб друг другу:

(A) ДЕШЁВЫЕ НАХОДКИ -- СРАЗУ В БАНК. Быстрый recon (file, strings, что за бинарь,
    какой вход он ждёт, какие защитные ветки видны в дампе/трассе) стоит копейки и
    даёт ГАРАНТИРОВАННЫЙ результат. Каждую такую находку фиксируй re-note
    НЕМЕДЛЕННО. Сессия может оборваться в любой момент -- к этому моменту в
    findings.jsonl уже должно что-то лежать. НЕ пропускай recon и НЕ жертвуй им
    ради спешки в динамику.

(B) ДИНАМИКУ -- РАНО, НЕ В КОНЦЕ. Главный приз (истинные значения констант)
    статикой НЕДОСТУПЕН -- он появляется только в памяти работающего процесса.
    Поэтому НЕ углубляй ДОРОГУЮ статику (полная декомпиляция всех функций ради
    полноты) -- это и есть та ловушка, что съедает бюджет впустую. Доводи статику
    ровно до того, чтобы (а) понять валидный вход и (б) локализовать
    функции-геттеры (адреса), и переходи к рантайм-дампу.

Порядок действий:
  1. Дешёвый recon -> re-note КАЖДОЙ находки сразу (принцип A). Это первые же шаги.
  2. Лёгкая статика: валидный вход и адреса функций-геттеров (не углубляясь в
     полную декомпиляцию).
  3. Рантайм-дамп -- ТВОЁ САМОЕ ЦЕННОЕ ДЕЙСТВИЕ, делай его в ПЕРВОЙ ТРЕТИ сессии.
     Подними бинарь и сними значения дампом (рецепт ниже). Есть живая Pi -- дампь
     на ней (часть защит на qemu даёт мусор, см. раздел про Pi). Каждое снятое
     значение -- немедленный re-note.
  4. Оставшееся время -- вглубь: прочие геттеры, алгоритмы, защитные механизмы.
Баланс: лучше рано снять несколько НАСТОЯЩИХ значений, чем идеально разобрать
статику и не дойти до динамики. Но в динамику иди НЕ с пустыми руками -- дешёвый
recon (шаг 1) банкуй всегда: если бюджет кончится на настройке дампа, у тебя
останутся хотя бы эти находки и собранный из них отчёт.
...
Динамику (запуск, gdb, рантайм-дамп) веди на живой Pi -- под qemu защита даёт
мусор. Не забудь LD_LIBRARY_PATH к каталогу с библиотеками. И помни правило
стратегии: переноси работу на Pi и снимай значения РАНО, в первой трети сессии,
не откладывая на конец -- иначе окно кончится на статике, и приз останется невзятым.
"""
...
def build_parser():
    ...
    ap.add_argument("--task", default=(
        ...
        "восстанови ключевые алгоритмы. Истинные значения статикой не видны -- их "
        "надо снимать в рантайме (gdb-дамп геттеров, на живой Pi если подключена). "
        "Дешёвый recon банкуй сразу (re-note каждой находки), а дорогую статику не "
        "растягивай: локализуй геттеры и переходи к рантайм-дампу РАНО, в первой "
        "трети сессии. Каждое извлечённое значение сразу фиксируй через re-note с "
        "адресом функции, где оно вычисляется."))
