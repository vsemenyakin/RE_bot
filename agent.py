...
ПОРЯДОК РАБОТЫ: ДИНАМИКУ -- РАНО, НЕ В КОНЦЕ. Это главное правило стратегии.
Частая и дорогая ошибка -- растянуть статический разбор на всю сессию и не успеть
снять значения в рантайме. У защищённого бинаря главный приз (истинные значения
констант) статикой НЕДОСТУПЕН -- он появляется только в памяти работающего
процесса. Значит статика сама по себе цель не берёт, и доводить её до полноты
бессмысленно. Порядок действий:
  1. Статики -- МИНИМУМ: ровно чтобы (а) понять валидный вход и (б) локализовать
     функции-геттеры (их адреса). Как только адреса есть -- НЕ углубляй статику,
     сразу переходи к п.2.
  2. Рантайм-дамп -- ТВОЁ САМОЕ ЦЕННОЕ ДЕЙСТВИЕ, делай его в ПЕРВОЙ ТРЕТИ сессии,
     а не под конец. Подними бинарь и сними значения дампом (рецепт ниже). Если
     подключена настоящая Pi -- дампь на ней (часть защит на qemu даёт мусор, см.
     раздел про Pi). Каждое снятое значение -- немедленный re-note.
  3. Оставшееся время -- вглубь: прочие геттеры, алгоритмы, защитные механизмы.
Сессия может оборваться в любой момент. Лучше рано снять несколько НАСТОЯЩИХ
значений, чем идеально разобрать статику и не дойти до динамики -- тогда приз
останется невзятым (так был потерян прошлый прогон: весь бюджет ушёл на recon).
...
def build_parser():
    ...
    ap.add_argument("--task", default=(
        ...
        "Не растягивай статику: локализуй геттеры и переходи к рантайм-дампу РАНО, "
        "в первой трети сессии. Каждое извлечённое значение сразу фиксируй через "
        "re-note с адресом функции, где оно вычисляется."))
