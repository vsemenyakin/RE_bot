...
- **`targets_gen.py`** — deterministic, no LLM. Scans `truth/` for `@re-target-*` markers and emits `protected.yaml`. Target `id` = the name in the code; renaming a protected symbol breaks the regression chain unless `id=` is pinned in the marker. A marker's `ignore-for="dev,mixed"` drops that target for those build configurations (a defense compiled out of a soft build must not be scored where it does not exist — otherwise it counts as "withstood" and inflates the soft build's resilience). `--build-configuration <name>` (fed from `RE_args.txt`) selects the active set, is stamped into `protected.yaml`, and becomes a comparability axis (see Attack fingerprint above). Empty/absent config → all targets (backward compatible).
...
**Attack fingerprint**: `attack_fingerprint()` stamps each run with `profile` + `prompt_sha` + `task_sha` + route-specific fields. **Comparability** (in `judge.py find_previous`) requires three axes to match before a delta is computed: same `attack_fingerprint`, same `judge_model`, and same `build_configuration`. A mismatch on any axis → the run is *incomparable* (labeled with the reason, e.g. "другой судья и другая конфигурация сборки"), never a delta — because the score depends on the attack strength, on *who judged* (a rubber-stamping model vs a reasoning one shift the baseline), and on *which build* was measured (dev/ship have different target sets). Version is orthogonal: among comparable runs, a different version yields the regression delta, the same version is measurement spread. Missing/empty `judge_model`/`build_configuration` normalize so pre-existing runs stay comparable to each other.

### Configuration split

- **`RE_args.txt`** — the session only: `sample`, `deps`, `attack_models` (comma-separated desc paths), `judge_model` (one desc path or litellm name), `label`, `build-configuration` (which build is in `samples/` — routes to `targets_gen.py --build-configuration`, gates `ignore-for` targets, and is a comparability axis), truth paths.
